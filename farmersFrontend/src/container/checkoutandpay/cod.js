...
function CodCheckout() {
  ...
  const { line1, city, pincode, error, success } = values;
  ...
  const Coduser = (user) => {
    ...
    return fetch(`http://localhost:3001/api/Codpayment`, {
      method: "POST",
      headers: {
  ...
  const showSuccess = () => (
    ...
    >
      <p>Thank You ,Please check your orders from order page</p>
    </div>
  );
  const clickSubmit = (event) => {
    event.preventDefault();
    setvalues({ ...values, error: false, loading: true });
    Coduser({ line1, city, pincode }).then((data) => {
      if (data?.message) {
    ...
    return (
      <div>
        <div className="payment_container">
          <div className="forms-payment_container">
            <div className="payment_container_cod-online">
              <form action="#" class="payment-in-form">
                <h2 className="title__payment">Cash On Delivery</h2>
                {showError()}
                {showSuccess()}
                <div className="payment__input-field">
                ...
                <button
                  onClick={clickSubmit}
                  type="submit"
                  className="btn payment__btn solid"
                >
                  Done
                </button>
                <p className="social-text">
                  Please be precise while adding location
                </p>
              </form>
            </div>
          </div>

          <div className="panels-payment_container">
            <div className="payment__panel payment__left-panel">
              <div className="content">
                <h3>Digital??</h3>
                <p>
                  Having a Card to pay !! <br />
                  trust Us
                </p>
                <NavLink to="/checkout">
                  <button
                    className="btn payment__btn  transparent"
                    id="payment-up-btn"
                  >
                    card pay
                  </button>
              ...
              >
                <defs>
                  ...
                  <linearGradient
                    ...
                    xlink
                    href="#8cd9664d-1da2-4d12-b141-6acf96f68a9c"
                  />
                  <linearGradient
